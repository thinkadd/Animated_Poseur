...
/**
 * This class stores some state information regarding the canvas rendering. Note
 * that we'll use one of these for each canvas. One for the true view, and one
 * for the zoomable view, so two in all.
 *
 * @author Xiufeng Yang
 * @version 1.0
 */
public class PoseCanvasState {
    ...
    /**
     * This constructor initializes this object with default values, but does
     * not set it up ready for use. It will only be ready for use after the
     * canvas has been made visible and has then been loaded into this object
     * using the loadPoseCanvas method.
     *
     * @param initPose The pose being manipulated by the user and rendered in
     * the pose ares.
     *
     * @param initZoomLevel The zoom level to use for rendering.
     *
     * @param initZoomFactor The scale to use in increasing and decreasing the
     * zoom level.
     *
    ...
    // ACCESSOR METHODS
    /**
     * Accessor method for getting the canvas associated with this canvas state.
     *
    ...
    /**
     * Accessor method for getting the pose used by the canvas for rendering.
     *
    ...
    /**
     * Accessor method for getting the rectangle on the canvas where we'll be
     * rendering the pose.
     *
     * @return The rectangle where the pose will be rendered.
     */
    public Rectangle2D.Double getPoseArea() {
        return poseArea;
    }

    /**
     * Accessor method for getting the zoom level for this canvas state.
     *
     * @return The zoom level, where 1 means the true state, greater than 1 is
     * zoomed in, and less than 1 is out.
     */
    ...
    /**
     * Accessor method for testing if this canvas can be zoomed or not.
     *
     * @return true is returned if this canvas can be zoomed, false otherwise.
     */
    ...
    /**
     * This mutator method sets the reference to the canvas for which this state
     * is associated.
     *
    ...
    /**
     * This method updates the values representing the pose area according to
    ...
    /**
     * This method zooms in the view on the canvas. Note that it will not zoom
     * so far that the pose area is larger than the canvas. After updating the
     * zoom level, this method will update the canvas view.
     */
    public void zoomIn() {
        ...
        // IN THE CANVAS
        int testZoomedPoseAreaWidth =
                (int) (pose.getPoseWidth() * testZoomLevel);
        ...
        int testZoomedPoseAreaHeight =
                (int) (pose.getPoseHeight() * testZoomLevel);
    ...
    /**
     * This method zooms out the canvas, which in turn will update the canvas
     * view. Note that this method will not allow zooming out too far according
     * to the minimum zoom.
     */
    public void zoomOut() {
        // DON'T DO ANYTHING IF IT'S NOT ZOOMABLE
