...
/**
 * This class stores the state information needed for the application's color
 * pallet, including what colors are currently in the pallet and where new
 * custom colors will be placed.
 *
 * @author Xiufeng Yang
 * @version 1.0
 */
public class ColorPalletState {
    ...
    /**
     * This constructor provides does not initialize the data for use for the
     * color pallet. To initialize the data, the loadColorPallet method must be
     * called following construction.
     *
     * @param initView The GUI component that will represent this color pallet.
     */
     ...
     */
    public Color getColorPalletColor(int index) {
        return colorPalletColors[index];
    }
    ...
    public int getColorPalletSize() {
        return colorPalletColors.length;
    }

    /**
     * Gets the number of rows in the color pallet.
     *
     * @return Number representing the number of rows of colors in the color
     * pallet.
     */
    public int getColorPalletRows() {
        return colorPalletRows;
    }

    /**
     * Gets the index of where a custom color is to be added next.
     *
     * @return The custom color index, which is where a custom color would be
     * added next to the pallet.
     */
     ...
     *
     * @param initView The component that will be used to rendering this color
     * pallet.
     */
    ...
    /**
     * This method sets up the color pallet for use. It loads all the custom
     * values at once.
     *
     * @param initColorPalletColors This array should be preloaded with all the
     * Colors necessary for the pallet.
     *
     * @param initColorPalletRows This represents the number of rows the color
     * pallet will be split into.
     *
     * @param initNumberOfFixedColors This number represents the number of
     * colors in the pallet that are set, and cannot be changed. The remaining
     * colors can be replaced by a custom color.
     *
     * @param initUnassignedCustomColor This is the Color to use in the pallet
     * for custom slots before the user has chosen a custom color to put there.
     */
    ...
    /**
     * This method will put the customColor argument into this application's
     * color pallet, and then force an update of the view. Note that we will use
     * a round robin algorithm for adding custom colors. We'll never touch the
     * colors loaded from the XML file, but if the pallet is full, the one
     * provided by this method will replace the oldest custom color in the
     * pallet.
     *
