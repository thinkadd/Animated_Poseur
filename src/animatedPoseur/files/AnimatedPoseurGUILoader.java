...
/**
 * This class provides services for loading settings from a properly formatted
 * XML file into a AnimatedPoseurGUI. At the moment, it only loads window
 * settings, but it could be extended to provide additional functionality.
 *
 * @author Xiufeng Yang
 * @version 1.0
 */
public class AnimatedPoseurGUILoader {

    /**
     * THE DEFAULT CONSTRUCT DOESN'T INITIALIZE ANTHING. THE initWindow METHOD
     * DOES ALL THE WORK. NOTE THAT THIS SIMPLE LITTLE CLASS IS JUST A
     * DEMONSTRATION OF HOW GUI SETTINGS CAN BE LOADED DYNAMICALLY.
     */
    public AnimatedPoseurGUILoader() {
    }

    /**
     * This method loads the settings found in the setupFileXML file, which must
     * be an XML file that validates against the gui_app_settings schema. The
     * loaded settings are then used to initialize the window argument.
     *
     ...
     */
    public void initWindow(AnimatedPoseurGUI window, String setupFileXML) {
        try {
            // THIS WILL DO OUR VALIDATION FOR US
            XMLUtilities xmlUtil = new XMLUtilities();

            // LOAD THE XML FILE INTO A DOC.
            Document setupDoc = xmlUtil.loadXMLDocument(
                    setupFileXML, WINDOW_SETTINGS_SCHEMA);
