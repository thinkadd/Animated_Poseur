...
/**
 * This component serves as a color pallet for selecting preset and custom
 * colors in an editing application. Note the corresponding ColorPalletState
 * class contains data and methods for initializing the colors in the pallet and
 * for updating them. When updated through that class, this view is refreshed.
 *
 * Note that this class has a symbiotic relationship with the ColorPalletState
 * class, they each have references to the other.
 *
 * @author Xiufeng Yang
 * @version 1.0
 */
public class ColorPallet extends JPanel {
    ...
    /**
     * This constructor sets up the pallet for use. Note that the
     * ColorPalletState argument must be fully loaded with the colors for the
     * color pallet before this is called.
     *
     * @param initColorPalletState The state information for this color pallet,
     * it must be pre-loaded so that this constructor can correctly make the
    ...
    /**
     * Accessor method for getting the state manager for this color pallet.
     *
    ...
    /**
     * Called whenever the color pallet state changes, this method reflects the
     * state data by updating the background colors of all the colors in the
     * state.
     */
    ...
    /**
     * Called when the GUI is setup, this method registers the handler for the
     * color pallet buttons. Note that all the color pallet buttons share this
     * one event handler.
     *
     * @param cph The event handler that will respond to when a user clicks on a
     * color pallet button.
     */
    ...
    /**
     * This method is used to enable and disable this color pallet, which fill
     * cascade this setting through all the buttons in the pallet.
     *
     * @param isEnabled If true is provided all the buttons in the pallet will
     * be enabled. If false is provided, all the buttons will be disabled.
     * Disabled buttons will not trigger events when one clicks on them and they
