...
/**
 * This is a custom component that we'll use for representing a toggle button
 * where the background color of the button will change depending on color
 * selection. The reason we don't just just JToggleButton is that we want the
 * color to show through even when the button is in the selected state.
 *
 * @author Xiufeng Yang
 * @version 1.0
 */
public class ColorToggleButton extends JButton {
    ...
    /**
     * Default constructor, by default it is not selected. The constructor also
     * sets up the border to use
     */
