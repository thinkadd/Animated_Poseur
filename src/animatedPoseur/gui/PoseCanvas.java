...
/**
 * A PoseCanvas will render the Pose in the center of its surface. Note that
 * these may be zoomed or not, depending on their state.
 *
 * @author Xiufeng Yang
 * @version 1.0
 */
public class PoseCanvas extends JPanel {
    ...
    /**
     * This constructor makes sure the canvas has the state from the beginning.
     *
     * @param initState The state to be used with rendering data for this
     * canvas.
     */
    ...
    /**
     * Accessor method for getting the state associated with this canvas.
     *
     * @return The canvas state object associated with this object, which
     * contains information about zoom levels and the pose area for rendering.
     */
    ...
    /**
     * This is where all the rendering of the canvas is done. Note that this
     * method breaks down rendering into the shapes already on the canvas, and
     * the shape in progress, which the user is working on.
     *
     ...
     * imageToPaintTo image argument. The paintComponent method paints to a
     * canvas (i.e. the screen), which this one is paints to an image that can
     * then be saved to a file
     *
     * @param imageToPaintTo The image we will paint what is in this canvas'
     * pose area to.
     */
    ...
    // HELPER METHODS
    /**
     * This method renders all debug text in the canvas. Doing is useful in a
     * graphical application because it's tricky stopping the debugger while
     * rendering, so using this technique we can view variable data in the app
     * while it is running.
     *
    ...
    /**
     * This method fills in the background of the pose area, which is in the
    ...
    /**
     * Renders all the shapes in the corresponding canvas state object for this
     * canvas.
     *
    ...
    /**
     * Renders the shape that is currently being sized by the user as they drag
     * the mouse across the canvas. We render this separately because we have
     * not yet put it in the shapes list for the pose.
     *
     * @param g2 The graphics context for this canvas.
     *
     * @param poseArea The area in the middle of the canvas where the pose will
     * be rendered.
     */
