...
/**
 * This event handler responds to when the canvas is rendered for the first
 * time. When that happens, we know the size of the canvas, and so can update
...
public class PoseCanvasComponentHandler implements ComponentListener {
    ...
    /**
     * This method is called when the size of the canvas is determined and set.
     * We can respond by getting that value and using it to size the pose area.
     * Note that the canvas state knows how to update itself, so we'll just ask
     * it to do so.
     *
